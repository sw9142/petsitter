...
<body>
	...
	<h1 align="center">관리자 모드</h1>
    <h3 align="center">매칭내역 관리</h3>
    <br>

    <div align="center" class="search_bar">
        <form action="<%= contextPath %>/matchsearch.ad" method="get" style="display: inline-block;">
    ...
    <script>
    	$(function() {
			$(".list-area>tbody>tr").click(function() {
				...
				location.href = "<%= contextPath %>/adminMatchDetail.ad?mno=" + mno;
    ...
    <!-- 페이징바에서 < 를 담당 : 이전페이지로 이동 -->
    	...
    	<% if(currentPage != 1) { %>
        	<button onclick="location.href='<%= contextPath %>/matchsearch.ad?currentPage=<%= currentPage - 1 %>&location=<%=location%>&sdate=<%=sdate%>&edate=<%=edate%>&id<%=id%>'">&lt;</button>
        <% } %>
        ...
        <% for(int i = startPage; i <= endPage; i++) { %>
            ...
            <% if(i != currentPage) { %>
            	<button onclick="location.href='<%= contextPath %>/matchsearch.ad?currentPage=<%= i %>&location=<%=location%>&sdate=<%=sdate%>&edate=<%=edate%>&id<%=id%>'"><%= i %></button>
            <% } else { %>
