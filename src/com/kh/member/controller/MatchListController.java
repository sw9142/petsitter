...
/**
 * Servlet implementation class MatchListController
 */
@WebServlet("/matchList.ma")
public class MatchListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Match> list = new MatchService().selectList(pi, loginUser);
		
		request.setAttribute("list", list); // 우리가 실제로 조회한 한 페이지에 보일 10개의 게시글
		request.setAttribute("pi", pi); // 페이징바를 만들때 필요한 변수
		
		request.getRequestDispatcher("views/member/matchList.jsp?currentPage=1").forward(request, response);
	}
